...
Base on this analysis, we defines three intrinsic features of a UAS operation to evaluated the likelihood of privacy violation case of ``UAS operation out of control":
\begin{itemize}
    \item Density of operational area: urban zone vs. rural zone
    \item Type of operation: BLOS vs. VLOS
    \item Level of detail of captured image. 
...
\subsection {PRC determination step}
In this step, the likelihood of privacy violation in case of "UAS operation out of control" is represented qualitativement by the PRC value. The intrinsic PRC value is determined based on the intrinsic features of operation as shown in Table \ref{intrinsic PRC}. 
\begin{table}[!ht]
\centering
\begin{tabular}{|
...
\label{intrinsic PRC}
\end{table}
Then the final PRC is determined based on the availability of three harm barriers:``Privacy protection filters", ``Restriction on private space" and `Operation-aware announcement to public`". Each means of mitigation helps decreases the intrinsic PRC factor one level. 
